...
// Update the ALLOWED_COMMANDS array to include more basic commands
...
// Improve the isCommandAllowed function to better handle path traversal
function isCommandAllowed(command) {
  ...
  if (baseCommand === "cd") {
    ...
    if (args.length > 1) {
      ...
      if (targetDir.includes("..") || targetDir.startsWith("/") || targetDir.match(/^[A-Z]:\\/i)) {
        return false
      }
    }
  }
...
router.post("/exec/:projectId", (req, res) => {
  ...
  // Enhance the CD command handling in the router.post("/exec/:projectId") route
  // Find the section that handles CD command and replace it with:
  ...
  if (command.startsWith("cd ")) {
    ...
    if (targetDir.startsWith("/") || targetDir.match(/^[A-Z]:\\/i)) {
      ...
      newCwd = path.resolve(projectPath, "..")

      // Security: Ensure we're still within the project directory
      if (!newCwd.startsWith(TEMP_DIR)) {
        return res.json({
